...
void rotation (SDL_Surface* Start_surface, SDL_Surface* Dest_surface, int degre)
{
    ...
    for (int x = 0; x<Start_surface->w; x++)
    {
        ...
        {
            Dpixels[y * Dest_surface->w + (Start_surface->w-1-x)]=Spixels[x * Start_surface->w + y];
        }
    }
...
int main(int argc, char** argv)
{
    ...
    //Probleme à regler sur la création de la deuxieme surface (size porbablement)
    SDL_Surface* Dest_surface = NULL;
    Dest_surface->w = Start_surface->h;
    Dest_surface->h = Dest_surface->w;
    ...
    rotation(Start_surface,Dest_surface,degree);


    SDL_Texture* texture = SDL_CreateTextureFromSurface(renderer, Dest_surface);
    if (texture==NULL)
	    errx(EXIT_FAILURE, "%s", SDL_GetError());
    //J'attribue la nouvelle surface avec les pixel tourner à l'ancienne surface
